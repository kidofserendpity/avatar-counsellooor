...
function addJournalFact(memory, fact) {
  const journalFacts = [...memory.journalFacts, fact].slice(-MAX_JOURNAL_FACTS);
  return { ...memory, journalFacts };
}
...
function buildMemoryBlock(memory, checkInNote = '') {
  const gapLine = getTimeGapLine(memory.lastMessageTimestamp);
  const factsLine = memory.facts.length
    ...
    : '';
  const journalLine = memory.journalFacts.length
    ? `Things you know from their private journal (never bring these up first — only engage with them if the person brings it up themselves, e.g. "about what I wrote..."): ${memory.journalFacts.join('; ')}.`
    : '';
